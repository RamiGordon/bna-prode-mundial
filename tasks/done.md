...
## T-009 — Migraciones SQL: tablas + RLS + triggers según `docs/03-data-model.md` (2026-05-27 → 2026-05-28)
- Resumen: schema inicial completo aplicado al proyecto Supabase remoto vía CLI. Una migración única `supabase/migrations/20260528022319_initial_schema.sql` (440 líneas) con: enum `match_stage`, 7 tablas (`profiles`, `teams`, `matches`, `predictions`, `bonus_predictions`, `result_changes_log`, `app_config`), 7 funciones (`is_admin`, `handle_new_user`, `set_updated_at`, `log_result_change`, `calculate_match_points`, `trigger_calculate_match_points`, `recalculate_bonus_points`), 5 triggers (1 en `auth.users`, 2 en `matches`, 1 en `predictions`, 1 en `bonus_predictions`), 15 policies de RLS, view `ranking` con `security_invoker=true`, y 1 fila seedeada en `app_config` con `tournament_start_at = 2026-06-11 22:00 UTC` y `scoring_rules` jsonb derivado del doc 04. Verificación vía `psql` directo a la connection string Direct: 7 tablas, RLS encendido en todas, 15 policies, scoring_rules con las 3 secciones, 7 funciones, 5 triggers. Build + lint verdes (sanity check, esta tarea no toca TS).
- Decisiones tomadas:
  - **Workflow = Supabase CLI con `db push` directo a remoto, sin Docker.** Sin DB local. Las migraciones viven en `supabase/migrations/` y se aplican con `supabase db push` linkeado al proyecto remoto via `supabase link --project-ref $SUPA_REF` (la var ya estaba en `.env.local` desde T-005). Trade-off: no hay sandbox local para probar cambios antes de pushear; aceptable para un proyecto de 20 users + 1 dev.
  - **Migración única** `<timestamp>_initial_schema.sql` con todo (tipos + tablas + funciones + triggers + RLS + view + app_config). Más limpio que separar por dominio cuando todo es coherente entre sí. Las próximas migraciones (incluidos seeds) van como archivos separados.
  - **Admin bootstrap = manual off-band**, no via trigger. Después del primer login (ya hecho en T-006), Rami debe correr `update profiles set is_admin = true where id = '<su uuid>'` en el SQL editor del dashboard. Alternativas descartadas: hardcodear email en `handle_new_user` (acopla la migración a un email específico, frágil) o insertar profile pre-creado en la migración (depende de coordinar UUIDs). El path manual es trivial y se hace una sola vez. **Pendiente off-band registrado en `in-progress.md`.**
  - **`calculate_match_points` implementada completa**, no stub. Lee puntos base + multiplicadores desde `app_config.scoring_rules` (jsonb), lo cual deja la fórmula configurable sin tocar código. Lógica en orden de prioridad: exacto → misma diferencia de goles (incluye ganador) → mismo signo (ganador o empate) → 0. T-022 después agrega tests automatizados + espejo TS.
  - **Helper `is_admin(uid uuid)` con SECURITY DEFINER + `search_path = public`** para evitar recursión cuando se usa en policies de la propia tabla `profiles`. Centraliza el check; las policies de admin no repiten subqueries.
  - **Redondeo de puntos con multiplicador: `round(numeric)::int`** (5 pts × 1.5 = 7.5 → 8). El doc 04 no lo aclara explícitamente; opté por round-half-up para no raspar puntos en r16/semis/tercer-puesto donde aparece el caso. En Postgres `round(numeric)` es half-away-from-zero, no bankers'.
  - **Trigger de recálculo dispara en `AFTER UPDATE OF (score_a, score_b, finalized_at)`**, no solo en `finalized_at` como decía el doc 03. Razón: el doc 04 dice "si un resultado se carga mal y se corrige, los puntajes se recalculan para todos" — si solo escuchara `finalized_at`, el admin tendría que re-tocar ese campo a mano para forzar el recálculo. Decisión documentada en `in-progress.md` antes de aplicar; divergencia consciente del doc 03 (que conviene actualizar en una pasada futura).
  - **Auditoría de cambios de resultado (`log_result_change`): `raise exception` si `auth.uid() is null`.** Privilegia auditoría sobre conveniencia operativa: no se pueden hacer cambios de resultado sin sesión autenticada. Esto bloquea cambios desde el SQL editor del dashboard sin user logueado, pero garantiza que `changed_by` (NOT NULL con FK a auth.users) siempre tenga valor real. Cambios excepcionales: vía service role o via el admin UI cuando esté T-024.
  - **Trigger genérico `set_updated_at()` BEFORE UPDATE** en `predictions` y `bonus_predictions` para mantener `updated_at` fresco. La regla del CLAUDE.md ("timestamps inmutables, no se borran ni al editar") implica que `updated_at` tiene que reflejar la edición real — sin el trigger queda como `created_at` salvo que el código TS lo setee a mano cada vez.
  - **View `ranking` con `security_invoker = true`** (PG15+) para que respete las RLS de las tablas subyacentes. Además reemplacé el `GROUP BY` del doc 03 (`group by p.id, p.alias, p.avatar_url, bp.points_awarded`, raro) por `max(bp.points_awarded)` con `group by p.id, p.alias, p.avatar_url`. Mismo resultado (válido porque `bonus_predictions.user_id` es PK → 0 o 1 fila por user), GROUP BY más limpio. Mejora respecto del doc.
  - **Stub vacío `recalculate_bonus_points()`**: firma declarada con body `return;`. La implementación real va en T-017+ cuando aterricen los bonus y al cierre del torneo. Lo dejé en esta migración para que la API quede declarada desde ya y no haya que mezclarla con seeds o lógica de bonus después.
  - **Smoke test de comportamiento de RLS movido a T-015**, no incluido en los criterios de aceptación de T-009. Razón: desde el SQL editor del dashboard se corre como rol `postgres` que bypassea RLS; el test ahí es poco representativo. T-015 ya está pensado para "verificar que RLS y server action rechazan submission post-kickoff" con seeds + user real, que es el contexto real.
  - **Tooling adicional instalado off-band:** Rami instaló `supabase` CLI (homebrew tap) y `libpq` (cliente Postgres con `psql`, sin server, linkeado con `brew link --force libpq`). Ambos quedan como dependencias de máquina de dev, fuera del repo.
- Bugs encontrados:
  - **Command Line Tools de Apple desactualizadas** bloqueaban el install de `supabase` via brew. Brew tiraba `Error: Your Command Line Tools are too outdated.` aunque el formula es bottled. Rami actualizó CLT via System Settings → Software Update (~700MB) y reintento OK. Es un tema de OS (macOS Tahoe / Darwin 25.3.0 recién actualizado), no del CLI; lo anoto por si vuelve a aparecer en futuras instalaciones via brew.
  - **`psql` no viene con macOS** y no lo trae el CLI de Supabase. Hubo que instalar `libpq` aparte para poder verificar el schema y para poder aplicar seeds en T-010/T-011 sin depender del SQL editor del dashboard. Ahora `psql 18.4` disponible globalmente.
  - **2 vulns moderate de postcss** siguen heredadas de Next, ya documentadas en T-004/T-006/T-007/T-008. Sin novedades.
- Pendientes off-band para Rami:
  - **Promover a admin manualmente** (descrito arriba). Necesario antes de T-024 (admin UI); no bloquea T-010/T-011 porque esos seeds los aplicamos vía `psql` con `SUPABASE_DATABASE_URL` (rol postgres bypassea RLS).
  - El de `nvm` en `.zshrc` (heredado de T-004) sigue abierto.
- Divergencias del `docs/03-data-model.md` a registrar (conviene rebuscarlo y dejarlo en sync en una próxima pasada): (1) trigger de recálculo escucha 3 columnas, no solo `finalized_at`; (2) view `ranking` con `max(bp.points_awarded)` en vez de incluir `bp.points_awarded` en el GROUP BY; (3) `log_result_change` exige `auth.uid()` not null. La nueva entrada del backlog para esto sería "T-009b: alinear `docs/03-data-model.md` con la migración aplicada" — la dejo anotada para mencionarte y meterla en `backlog.md` si querés en la próxima sesión.
- Próximo: **T-010 (seed de 48 selecciones del Mundial 2026)** sigue Sesión 3 / día 3.
- Commits relevantes: commit de cierre.

## T-010 — Seed de 48 selecciones del Mundial 2026 (2026-05-28)
- Resumen: migración `supabase/migrations/20260528041855_seed_teams.sql` con un único `INSERT INTO teams (name, code, group_letter, flag_emoji) VALUES (...)` de 48 filas, agrupadas y comentadas por grupo (A..L) en el orden del sorteo FIFA. Aplicada al remoto vía `supabase db push --include-all` (la CLI confirmó "Finished supabase db push"). Verificación con `psql`: 48 filas totales, 4 por cada grupo A..L, sample por código (ARG/BRA/ENG/SCO/USA/KSA) con bandera y nombre correctos.
- Decisiones tomadas:
  - **IDs serial automático**, sin id explícito en el INSERT. Robusto ante `db reset`: si los ids cambian, T-011 sigue funcionando porque referencia por `code` (`(select id from teams where code = 'ARG')`). Alternativa descartada: id hardcoded + `setval` final — más frágil y sin upside.
  - **Nombres en español rioplatense según el listado pasado por Rami**: "México", "Sudáfrica", "Corea del Sur", "República Checa", "Bosnia-Herzegovina", "Países Bajos", "Estados Unidos", "Países Bajos", "Cabo Verde", "Arabia Saudita", "RD Congo". Se evitaron variantes ambiguas ("EE.UU.", "Holanda", "Côte d'Ivoire"). El listado y la ortografía son fuente canónica de Rami para todas las pantallas (T-012+).
  - **Códigos FIFA, no ISO 3166-1 alpha-3**. Difieren en varios casos relevantes: `ALG` (no `DZA`), `KSA` (no `SAU`), `SUI` (no `CHE`), `URU` (no `URY`), `POR` (no `PRT`), `GER` (no `DEU`), `NED` (no `NLD`), `RSA` (no `ZAF`), `PAR` (no `PRY`), `HAI` (no `HTI`), `IRN` (no `IRN` — coincide), `URU` (no `URY`), `CRO` (no `HRV`), `BIH` (coincide). Para Escocia (`SCO`) e Inglaterra (`ENG`) la FIFA usa codes que ISO no tiene (subdivisión del Reino Unido). Los emojis de bandera de esos dos son los Unicode subdivision flags (🏴󠁧󠁢󠁳󠁣󠁴󠁿/🏴󠁧󠁢󠁥󠁮󠁧󠁿); pueden renderizar como bandera negra en Windows viejos — trade-off conocido, se sustituyen por SVG si algún tester reporta el problema.
  - **Incluir `flag_emoji` en este seed**, no diferir. Son texto Unicode trivial de mantener; deja T-012 lista para mostrar banderas sin migración extra.
  - **Migración separada** (un archivo por seed), no append al `20260528022319_initial_schema.sql`. Cada migración timestamped es inmutable una vez aplicada; los seeds vienen claramente etiquetados como tales en el nombre.
  - **`supabase db push --include-all`**: flag necesario porque el CLI por default sólo pushea las migraciones más recientes que el último timestamp aplicado al remoto, pero quería forzar que detectara nuestra nueva en caso de cualquier desfasaje. La aplicación fue idempotente y limpia.
- Bugs encontrados:
  - **2 vulns moderate de postcss** siguen heredadas de Next, ya documentadas en T-004 a T-009. Sin novedades.
- Pendientes off-band para Rami: ninguno nuevo. Los heredados (`nvm` en `.zshrc` desde T-004, promover a admin manualmente desde T-009) siguen abiertos pero no bloquean T-011 (mismo motivo: seeds vía `psql`/`supabase db push` bypassean RLS).
- Próximo: **T-011 (seed de 104 partidos con fixture oficial FIFA, fechas en UTC)** sigue Sesión 3 / día 3.
- Commits relevantes: commit de la migración + commit de cierre.
